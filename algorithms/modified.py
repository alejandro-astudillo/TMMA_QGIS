...
from shared.paint import paint_snapped_point, paint_buffer, dibujar_ruta_con_flechas, dibujar_flechas_gps_snapped

import math

class ModifiedTMMA(MapMatchingStrategy):
    """Implementación del algoritmo de Map Matching Topológico Modificado."""
    ...
    def run(self):
        ...
        while i < len(puntos_gps_list) and j < len(puntos_gps_list):
          ...
          if len(calles_intersectantes_punto_j) == 0:
            calle_punto_anterior_a_j = self.geometrias[punto_i_id]['assigned_route']
            if calle_punto_anterior_a_j is None:
              j += 1
              continue
            geom_punto_snapped_j = project_point(punto_j, calle_punto_anterior_a_j)
            self.geometrias[punto_j_id]['geom_punto_snapped'] = geom_punto_snapped_j
            self.geometrias[punto_j_id]['assigned_route'] = calle_punto_anterior_a_j
          ...
          # -*- Verificar Ruta Asignada de punto Anterior -*-
          calle_mas_cercana_punto_j = None
          calle_mas_cercana_punto_i = self.geometrias[punto_i_id]['assigned_route']
          if calle_mas_cercana_punto_i is not None and calle_mas_cercana_punto_i in calles_intersectantes_punto_j:
            calle_mas_cercana_punto_j = calle_mas_cercana_punto_i
            route_validation = self._is_valid_route(punto_i, punto_j, calle_mas_cercana_punto_i, calle_mas_cercana_punto_j)
            if route_validation == "giro":
              i = j
              j = i + 1
              continue
            if route_validation:
          ...
          while consecutive_points <= self.max_consecutive_points:
            ...
            if calle_mas_cercana_punto_j is not None and calle_mas_cercana_punto_j in calles_intersectantes_punto_j:
            ...
            while len(calles_intersectantes_punto_i_copy) > 0:
    ...
    def _is_valid_route(self, punto_i, punto_j, calle_mas_cercana_punto_i, calle_mas_cercana_punto_j):
      ...
      # -*- Calcular Angulo entre puntos snapped -*-
      angle_deg = None
      if geom_punto_snapped_i is not None and geom_punto_snapped_j is not None:
          p1 = geom_punto_snapped_i.asPoint()
          p2 = geom_punto_snapped_j.asPoint()
          dx = p2.x() - p1.x()
          dy = p2.y() - p1.y()
          if dx != 0 or dy != 0:
              angle_deg = (math.degrees(math.atan2(dy, dx)) + 360) % 360

      # -*- Detectar giro (diferencia de orientaciones) -*-
      turn_made = None
      if angle_deg is not None and self.last_heading_deg is not None:
          diff = abs(angle_deg - self.last_heading_deg)
          if diff > 180:
              diff = 360 - diff
          turn_made = diff >= self.angulo_giro_umbral
          if turn_made:
            self.last_heading_deg = None
            return "giro"

      # Actualizar heading actual para el siguiente segmento
      if angle_deg is not None:
          self.last_heading_deg = angle_deg
    ...
    def _save_results(self, puntos_gps_list: list):
      ...
      output_file_path = os.path.join(output_results_dir, f"{file_name_without_ext_for_output}_results_MODIFIED_BUFF_{int(self.config['buffer_distance_ft'])}_FRECUENCY_{self.gps_points_frecuency}.txt")
      ...
      with open(output_file_path, 'w', encoding='utf-8') as outfile:
          ...
          outfile.write(f"ANGULO GIRO UMBRAL: {self.config['angulo_giro_umbral']}" + "\n")
